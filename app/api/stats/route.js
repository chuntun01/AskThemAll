...
export async function GET(req) {
  try {
    ...
    const dailyAgg = await Question.aggregate([
      ...
      {
        $group: {
          _id: { $dateToString: { format: "%d-%m-%Y", date: "$createDate", timezone: TIMEZONE } },
          count: { $sum: 1 },
        },
      },
      { $sort: { _id: 1 } },
    ...
    const byModelAgg = await Answer.aggregate([
      ...
      {
        $lookup: { from: "aimodels", localField: "_id", foreignField: "_id", as: "model" }
      },
    ...
    const byModel = byModelAgg.map(m => ({ name: prettyName(m.rawName), count: m.count }));
